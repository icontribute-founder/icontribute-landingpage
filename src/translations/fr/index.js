export const TRANSLATIONS_FR = {
  // NAVBAR
  HOME: "ACCUEIL",
  VOLUNTEERS: "BÉNÉVOLES",
  ORGANIZATIONS: "ORGANISATIONS",
  "OUR TEAM": "NOTRE ÉQUIPE",
  SubLogo: "Nous nous élévons en élevant les autres",

  // FOOTER
  "CONTACT US": "CONTACTEZ-NOUS",
  "TERMS & CONDITIONS": "CONDITIONS GÉNÉRALES",
  "PRIVACY POLICY": "POLITIQUE DE CONFIDENTIALITÉ",
  "FOLLOW US": "SUIVEZ-NOUS",
  "Rights Text": " iContribute. Tous droits réservés.",

  // HOMEPAGE
  "Home-MainBlock-Header": "Le bénévolat devenu plus facile.",
  "Home-MainBlock-Details":
    "Nous facilitons la recherche, la gestion et le jumelage des participantes et des participants aux occasions de bénévolat pour les élèves, ou les étudiantes et étudiants et les organisations.",
  "StripedTip-1": "Facile à inscrire en quelques étapes",
  "StripedTip-2": "Gratuit pour les organisations bénévoles et les étudiants",
  "StripedTip-3":
    "Établissez de nouvelles connexions et développez votre réseau",
  Fundraiser: "Campagne de financement",
  "Ottawa Community": "Communauté de la capitale nationale",
  "Sat Jun 12, 2020": "Samedi 12 Juin 2020",
  "Sat Jun 16, 2020": "Samedi 16 Juin 2020",
  "Sat Jun 17, 2020": "Samedi 17 Juin 2020",
  "photoblock-container-header":
    "Apportez une contribution significative à votre communauté.",
  "BookDemo": "RÉSERVER UNE DÉMO",
  "demoblock-header-0": "Une plateforme",
  "demoblock-header-1":
    "Une plateforme unique pour les élèves, les étudiantes ou les étudiants et les organisations.",
  "demoblock-disc-1":
    "Nous mettons en relation les gens qui cherchent des occasions de bénévolat locales ou virtuelles avec des organisations à but non lucratif qui recrutent activement.",
  "demoblock-subheader-2":
    "Des étapes toutes simples pour vous lancer rapidement.",
  "demoblock-disc-2": "Voilà comment. Ouais. C'est facile comme ça!",
  "demostep-header-3": "INSCRIPTION",
  "demostep-disc-3":
    "Donnez-nous quelques détails à votre sujet ou au sujet de votre organisation.",
  "demostep-header-4": "EXPLORATION",
  "demostep-disc-4":
    "Les bénévoles parcourent et postulent à des occasions créées par les organisations.",
  "demostep-header-5": "CONNEXION",
  "demostep-disc-5": "Nous vous connectons en votre nom.",
  "partners-header": "Nos partenaires",
  "partners-desc": "Trouvez ce qui vous passionne.",
  "backed-by-header": "Soutenu par",
  "VideoBlock-header": "Featured:",
  "FAQBlock-header": "Des questions?",
  "FAQBlock-desc-1.1": "Envoyez-nous vos questions ",
  "FAQBlock-desc-1.2": "Nous sommes toujours prêtes et prêts à vous appuyer.",
  "FAQBlock-link": "ici",
  "GetInTouchBlock-Header": "COMMUNIQUEZ AVEC NOUS",
  "GetInTouchBlock-Subheader":
    "N'hésitez pas à nous contacter. Nous sommes heureux de vous aider.",
  CONTACT: "CONTACTEZ-NOUS",

  // ORGANIZATION PAGE
  "Organizations-MainBlock-Header": "Touchez un public plus large.",
  "Organizations-MainBlock-Details":
    "Votre organisation peut augmenter sa visibilité dans la communauté d'Ottawa en se connectant avec les élèves, les étudiantes et les étudiants via notre plateforme mobile. Il suffit de quelques étapes pour commencer.",
  "HOW IT WORKS": "COMMENT ÇA FONCTIONNE",
  "Connect to 1000s of students in the community":
    "Connectez-vous à des milliers d’élèves, d’étudiantes et d'étudiants de la communauté",
  STEP: "ÉTAPE",
  "Download the app": "Téléchargez l'application",
  "Download iContribute app for your mobile device. It works with iOS and Android.":
    "Téléchargez l'application iContribute sur votre appareil mobile. Il fonctionne avec iOS et Android",
  "Sign Up as an Organization": "Inscrivez-vous en tant qu'organisation",
  "Fill in the required fields to make an account with us. It only takes a few fields.":
    "Remplissez les champs obligatoires pour créer un compte chez nous. Cela ne prend que quelques minutes.",
  "Create and Post your Events": "Créez et publiez vos activités",
  "Fill in the details about your event and the time slots you would like volunteers to work.":
    "Ajoutez des détails au sujet de l’activité proposée et désignez les plages horaires pour lesquelles vous souhaitez que les bénévoles travaillent.",
  "Confirm Your Volunteers": "Confirmez la demande de vos bénévoles",
  "Volunteers will apply to your event and all you have to do is confirm if they will be attending via email.":
    "Les bénévoles postuleront à votre activité et tout ce que vous aurez à faire est de confirmer votre accord avec elles ou avec eux par courriel.",
  "Student Volunteer": "Étudiant bénévole",

  // VOLUNTEERS PAGE
  "Volunteers-MainBlock-Header":
    "Trouve des activités de bénévolat facilement.",
  "Volunteers-MainBlock-Details":
    "Tu peux commencer avec nous dès maintenant. Après avoir créé un compte avec nous, tu pourras parcourir une variété d’activités de bénévolat.",
  "Bubble-Header-1": "Découvre les activités près de chez toi",
  "Bubble-Caption-1":
    "Tu as accès à un large éventail d’occasions de bénévolat dans ta région.",
  "Bubble-Header-2": "Sauvegarde les activités de ton choix pour plus tard",
  "Bubble-Caption-2":
    "Tu peux sauvegarder les activités qui t’intéressent et y postuler ultérieurement.",
  "Bubble-Header-3": "Visionner les activités à venir",
  "Bubble-Caption-3":
    "Prévoit la suite de ton programme de bénévolat et visualise les activités que tu as déjà complétées.",
  "Bubble-Header-4": "Personnalise ton profil",
  "Bubble-Caption-4":
    "Modifie tes intérêts personnels pour nous aider à trouver les activités de bénévolat qui te conviennent.",
  "connect-header-h1":
    "Te brancher aux organisations se trouve au bout de tes doigts!",
  "connect-header-p":
    "Postule à une activité qui t’intéresse et nous rédigeons automatiquement un courriel pour que tu puisses communiquer directement avec l'organisation.",
  Testimonials: "Témoignages",

  // OUR TEAM
  "watermark-main":
    "Les personnes qui travaillent chez iContribute partagent la vision de notre communauté.",
  "AcrossGlobeBlock-header": "Depuis l'autre côté du continent",
  "AcrossGlobeBlock-subheader":
    "Notre équipe est parsemée dans plusieurs régions du monde. Nous avons collaboré sans cesse pour vous proposer notre application mobile.",
  "TeamBlock-Header-h1":
    "iContribute est créé et maintenu par ce merveilleux groupe d’étudiantes et d'étudiants.",
  "TeamBlock-Header-p": "Cliquez pour en savoir davantage à notre sujet!",
  "teamblockcall-container-h1":
    "Nous aimons beaucoup ce que nous faisons, mais nous sommes également attachés à l’équilibre travail-vie personnelle.",
  "teamblockcall-container-p":
    "Bien que nous soyons à des kilomètres l'un de l'autre, nous avons trouvé des moyens de demeurer connectés.",
};
